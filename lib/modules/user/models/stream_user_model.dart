...
class StreamUser extends User {
  ...
  final UserProviderStore assistidoStore;
  final StreamController<StreamUser> _chamadaController =
      StreamController<StreamUser>.broadcast();
  static final countPresenteController = RxNotifier<int>(0);
  StreamUser(super.assistido, this.assistidoStore) : super.assistido();
  StreamUser.vazio(this.assistidoStore, {int key = -1})
      : super(
            ident: key,
            nomeM1: "Nome",
            logradouro: "Rua",
            endereco: "",
            numero: "0");
  Stream<StreamUser> get chamadaStream => _chamadaController.stream;

  User get assistido => this;
  ...
  void copy(StreamUser? assistido) {
    if (assistido != null) {
